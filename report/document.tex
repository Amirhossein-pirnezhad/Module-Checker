...
\begin{itemize}
	...
	\item \lr{Expr} علاوه بر چهار سازنده‌ی مثال راهنما (\lr{num}، \lr{var}، \lr{add}، \lr{lt})، دو سازنده‌ی \lr{mul} و \lr{equal} هم اضافه شده تا زیرمجموعه‌ی پوشش داده شده به عملگرهای محاسباتی و مقایسه‌ای فاز اول نزدیک‌تر باشد.
...
\begin{itemize}
	...
	\item در شاخه‌ی \lr{whileTrue}، بعد از \lr{cases hEq} همان جایگزینی رخ می‌دهد. حالا از \lr{bodyPreservesInvariant} (با سه آرگومان \lr{hInv}، \lr{hCond} و \lr{hBody}) نتیجه می‌گیریم که ناوردا بعد از \textbf{یک بار} اجرای بدنه هم برقرار است: \lr{hInvAfterBody : P sMid}. سپس فرض استقرای مربوط به «ادامه‌ی حلقه» یعنی \lr{ihRest} را با دو آرگومان \lr{rfl} (اثبات بدیهی تساوی \lr{Stmt.while b body = Stmt.while b body}) و \lr{hInvAfterBody} صدا می‌زنیم تا \lr{P sFinal} نتیجه شود.
\end{itemize}

این الگو --- عمومی کردن یک گزاره با افزودن یک فرض تساوی صریح، پیش از استقرا --- در ادبیات اثبات‌یارها به «ترفند remember» معروف است (در \lr{Coq} معادل تاکتیک \lr{remember ... as ... eqn:...})، و هر جا بخواهیم از استقرا روی اشتقاق یک رابطه‌ی استقرایی که یکی از آرگومان‌هایش به یک عبارت \textbf{مرکب و ثابت} (نه یک متغیر ساده) گره خورده استفاده کنیم، ضروری است.
...
\dfrac{
	...
	\dfrac{
		...
		\quad
		\dfrac{\text{(شرط نادرست)}}{\langle \mathit{ta}(b)\{S\}, \sigma_2\rangle \Downarrow \sigma_2}\;\text{(whileFalse)}
	}{\langle \mathit{ta}(b)\{S\}, \sigma_1\rangle \Downarrow \sigma_2}\;\text{(whileTrue)}
}{\langle \mathit{ta}(b)\{S\}, \sigma_0\rangle \Downarrow \sigma_2}\;\text{(whileTrue)}
\]
...
در این فاز، با تعریف صوری \lr{Big-Step} برای زیرمجموعه‌ی \lr{Hash} و اثبات قضیه‌ی پایستگی ناوردای حلقه در \lr{Lean 4}، از سطح بررسی موردیِ فاز دوم (که فقط درباره‌ی یک برنامه‌ی خاص با \lr{SPIN} صحبت می‌کرد) به یک تضمین ریاضی و کلی رسیدیم که برای \textbf{هر} برنامه‌ی \lr{Hash} حاوی یک حلقه‌ی \lr{ta} با بدنه‌ی ناوردا-حفظ‌کننده صادق است. این قضیه نمونه‌ی کوچکی از همان رویکردی است که، طبق مقدمه‌ی صورت پروژه، شرکت‌هایی مانند \lr{Amazon} برای اثبات صحت زیرساخت‌های نرم‌افزاری‌شان به‌کار می‌برند: به‌جای تکیه بر تست‌های موردی، درستی را یک‌بار و برای همیشه، به‌صورت ریاضی، ثابت می‌کنند.
