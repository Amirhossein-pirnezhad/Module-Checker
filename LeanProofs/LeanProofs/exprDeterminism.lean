...
theorem stateEquivalence : forall S s1 s2 s1',
  ...
  | assign =>
      ...
      have he : evalExpr e s_curr = evalExpr e s2 :=
        exprAgree e s_curr s2 (fun y hy => hagree y (Or.inr hy))
      intro y hy
      unfold update
      by_cases hxy : y = x
      · subst hxy; simp [he]
      · simp [hxy]; exact hagree y hy
  | seq hb1 hb2 ih1 ih2 =>
      rename_i S1 S2 s_start s_mid s_end
      have hagree1 : forall x, freeVars S1 x -> s_start x = s2 x :=
        fun x hx => hagree x (Or.inl hx)
      obtain ⟨s2mid, hb2mid, hagreemid⟩ := ih1 s2 hagree1
      have hagree2 : forall x, freeVars S2 x -> s_mid x = s2mid x := by
        intro x hx2
        by_cases hx1 : freeVars S1 x
        · exact hagreemid x hx1
        · rw [<- unassignedUnchanged S1 s_start s_mid x hb1 hx1,
              <- unassignedUnchanged S1 s2 s2mid x hb2mid hx1]
          exact hagree x (Or.inr hx2)
      obtain ⟨s2', hb2', hagree'⟩ := ih2 s2mid hagree2
      refine ⟨s2', BigStep.seq hb2mid hb2', ?_⟩
      intro x hx
      by_cases hx2 : freeVars S2 x
      · exact hagree' x hx2
      · rw [<- unassignedUnchanged S2 s_mid s_end x hb2 hx2,
            <- unassignedUnchanged S2 s2mid s2' x hb2' hx2]
        by_cases hx1 : freeVars S1 x
        · exact hagreemid x hx1
        · rw [<- unassignedUnchanged S1 s_start s_mid x hb1 hx1,
              <- unassignedUnchanged S1 s2 s2mid x hb2mid hx1]
  ...
  | ifTrue hb hbody ih =>
      rename_i b S1 S2 s_curr s_end
      have hbEq : evalExpr b s2 ≠ 0 := by
        have hae := exprAgree b s_curr s2 (fun x hx => hagree x (Or.inl hx))
        rw [<- hae]; exact hb
      have hagree1 : forall x, freeVars S1 x -> s_curr x = s2 x :=
        fun x hx => hagree x (Or.inr (Or.inl hx))
      obtain ⟨s2', hb2', hagree'⟩ := ih s2 hagree1
      exact ⟨s2', BigStep.ifTrue hbEq hb2', fun x hx => by
        by_cases hx1 : freeVars S1 x
        · exact hagree' x hx1
        · rw [<- unassignedUnchanged S1 s_curr s_end x hbody hx1,
              <- unassignedUnchanged S1 s2 s2' x hb2' hx1]
          exact hagree x hx
      ⟩
  | ifFalse hb hbody ih =>
      rename_i b S1 S2 s_curr s_end
      have hbEq : evalExpr b s2 = 0 := by
        have hae := exprAgree b s_curr s2 (fun x hx => hagree x (Or.inl hx))
        rw [<- hae]; exact hb
      have hagree2 : forall x, freeVars S2 x -> s_curr x = s2 x :=
        fun x hx => hagree x (Or.inr (Or.inr hx))
      obtain ⟨s2', hb2', hagree'⟩ := ih s2 hagree2
      exact ⟨s2', BigStep.ifFalse hbEq hb2', fun x hx => by
        by_cases hx2 : freeVars S2 x
        · exact hagree' x hx2
        · rw [<- unassignedUnchanged S2 s_curr s_end x hbody hx2,
              <- unassignedUnchanged S2 s2 s2' x hb2' hx2]
          exact hagree x hx
      ⟩
  | whileFalse hb =>
      ...
      have hbEq : evalExpr b s2 = 0 := by
        have hae := exprAgree b s_curr s2 (fun x hx => hagree x (Or.inl hx))
        rw [<- hae]; exact hb
      exact ⟨s2, BigStep.whileFalse hbEq, hagree⟩
  | whileTrue hb hbody hrest ih1 ih2 =>
      rename_i b body s_start s_mid s_end
      have hbEq : evalExpr b s2 ≠ 0 := by
        have hae := exprAgree b s_start s2 (fun x hx => hagree x (Or.inl hx))
        rw [<- hae]; exact hb
      have hagreeS : forall x, freeVars body x -> s_start x = s2 x :=
        fun x hx => hagree x (Or.inr hx)
      obtain ⟨s2mid, hb2mid, hagreemid⟩ := ih1 s2 hagreeS
      have hagreeRest : forall x, freeVars (Stmt.while b body) x -> s_mid x = s2mid x := by
        ...
        · rw [<- unassignedUnchanged body s_start s_mid x hbody hxS',
              <- unassignedUnchanged body s2 s2mid x hb2mid hxS']
          exact hagree x hx
